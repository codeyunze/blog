...
> seata-server服务端服务注册到nacos里的命名空间可以和业务服务注册到的命名空间不同，分组不同。
>
> 但是业务服务，也就是seata客户端的 seata.registry 和 seata.registry 以及 seata.tx-service-group 、seata.service.vgroup-mapping.事务分组名=集群名称，这几个配置一定要和 seata 服务端application.yml里的配置一样。
